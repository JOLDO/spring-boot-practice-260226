...
public class Board_0306 extends BaseEntity_0306 {  //베이스 엔티티를 상속 받으면 됨
    ...
    // @OneToMany
    @OneToMany(mappedBy = "board", cascade = {CascadeType.ALL}, fetch = FetchType.LAZY, orphanRemoval = true)  //"board"는 BoardImage의 board 변수를 의미 하고 BoardImage가 주인이라는 뜻,  고아 객체 제거 속성 이용
    @Builder.Default
    @BatchSize(size = 20)
